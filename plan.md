...
This study plan is designed to turn theoretical knowledge into real architectural capability. Unlike packaged codelabs that hide operational complexity, this lab requires building the stack from scratch using Infrastructure as Code (IaC).

As architects, our goal is not only to "make it work," but to optimize the cost-performance ratio using accessible hardware such as NVIDIA L4 GPUs, while avoiding the bureaucracy and prohibitive costs of TPUs.

## Repository Structure

```text
/ai-platform-monorepo
├── 1-infra/                      # Pure Terraform
│   ├── vpc.tf                    # Custom VPC (Key: MTU 8896 for NCCL/GPU transfer)
│   ├── gke_cluster.tf            # GKE Standard cluster (Control plane)
│   └── node_pools.tf             # 1 CPU pool (system), 2 L4 GPU pools (prefill/decode) with taints
├── 2-platform/                   # Helm/Kustomize (Middleware layer)
│   ├── nvidia-device-plugin/     # DaemonSet to expose GPUs to Kubernetes
│   ├── kuberay-operator/         # Ray operator to orchestrate workers
│   └── gateway-api/              # GKE Gateway Controller CRDs (intelligent routing)
├── 3-llmops/                     # Observability and Operations
│   ├── kube-prometheus-stack/    # Metrics scraping
│   ├── dcgm-exporter/            # Data Center GPU Manager (temperature, VRAM, tensor cores)
│   └── grafana-dashboards/       # Custom vLLM dashboards (tokens/s, queue latency, KV cache usage)
└── 4-workloads/                  # Model deployment
    ├── rbac/                     # Workload Identity (permissions for GCS/Hugging Face secrets)
    ├── ray-cluster.yaml          # CRD defining the vLLM cluster (Head node, prefill workers, decode workers)
    └── inference-routing.yaml    # InferencePool and InferenceModel (Gateway routing)
```
...
- **Inference Extension API and kgateway:** We will implement routing through kgateway (Envoy-based). This layer is not a simple HTTP load balancer; it is an intelligent routing engine implementing the Endpoint Picker Protocol (EPP), enabling backend selection based on KV cache utilization and queue depth.
...
- `dcgm_gpu_temp`: Monitors thermal throttling.
- `vllm:kv_cache_usage_ratio`: Key health indicator. A 100% ratio means the system will begin evicting context or recomputing.
- `dcgm_tensor_copy_util`: Helps identify whether we are compute-bound.

### Grafana Dashboard

The dashboard design must include panels that correlate KV cache usage with queue latency. If latency rises while VRAM is full, we have identified a memory bottleneck that requires horizontal scaling or tiered storage policies.
...
- **Model Setup:** The workhorse model for this lab will be `Llama-3-8B-Instruct`. Although the infrastructure can serve Qwen3-32B or Llama-3.1-405B, the 8B model ensures load tests do not exceed budget.
...
According to GKE benchmarks, implementing Tier 3 (Local SSD) for long contexts of 100k tokens enables:
...
1. **Provisioning (15 min):** `terraform apply` to bring up the VPC (MTU 8896) and L4 node pools.
2. **Orchestration:** `kubectl apply` for platform manifests (KubeRay, kgateway).
3. **Deploy and Test (30 min):** Deploy Llama-3-8B and run load tests. Monitor VRAM saturation in Grafana.
4. **Immediate Teardown:** `terraform destroy`.

## Final Mandate

Do not persist resources. Infrastructure must be treated as ephemeral. Technical mastery comes from the ability to bring up and tear down this full stack in minutes, not from keeping it running.
